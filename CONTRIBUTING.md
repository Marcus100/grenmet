...
**Scope** is the app or package affected: `gaa-admin`, `gms`, `auth`, `ui`, `api-client`, `fastapi`, `ci`, etc.

**Examples:**

```
feat(gaa-admin): add midday forecast PDF export
...
- A legacy weather schema reference without the corresponding FastAPI contract or migration review
...
salesbus. FastAPI contract and migration changes require the generated client, API drift check, and domain tests. CI supplies the
...
### Database (owned by gaa-admin)

| Rule | Detail |
|---|---|
| FastAPI contract changes → always regenerate | After editing a FastAPI route or schema, regenerate `openapi.json`, run `pnpm generate:api-client`, and run `pnpm check:drift` |
