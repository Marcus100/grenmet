...
**Scope** is the app or package affected: `admin-gms`, `wxproducts`, `auth`, `ui`, `api-client`, `fastapi`, `ci`, etc.

**Examples:**

```
feat(wxproducts): add midday forecast PDF export
fix(auth): redirect loop when session cookie is missing
docs(env): add salesbus variables
...
```bash
turbo run check:fix type-check --filter=@grenmet/web-wxproducts
```
...
### Database (wxproducts / wxwatch)

| Rule | Detail |
|---|---|
| Schema changes → always regenerate | After editing `src/db/schema/`, run `pnpm db:generate` |
