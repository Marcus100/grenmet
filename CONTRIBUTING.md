...
| Rule | Detail |
|---|---|
| Schema changes → always regenerate | After editing `apps/web/gaa-admin/src/db/<family>/schema.ts` or `schema/`, run `pnpm db:<family>:generate` from `apps/web/gaa-admin` for janitorial, transport, wxproducts, or wxwatch |
