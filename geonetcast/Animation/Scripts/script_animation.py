...
while (date_loop <= date_end):
    ...
    if int(band) <= 6:
        ...
        cpt = loadCPT('../../shared/Square Root Visible Enhancement.cpt')
        ...
        # Converts a CPT file to be used in Python
        cpt = loadCPT('../../shared/SVGAIR2_TEMP.cpt')
        cmap = LinearSegmentedColormap('cpt', cpt) 
        data -= 273.15
        vmin = -112.15
        vmax = 56.85
        ...
        # Converts a CPT file to be used in Python
        cpt = loadCPT('../../shared/SVGAWVX_TEMP.cpt')
        cmap = LinearSegmentedColormap('cpt', cpt) 
        data -= 273.15
        vmin = -112.15
        vmax = 56.85
        ...
        # Converts a CPT file to be used in Python
        cpt = loadCPT('../../shared/IR4AVHRR6.cpt')   
        cmap = LinearSegmentedColormap('cpt', cpt) 
        data -= 273.15    
        vmin = -103.0
        vmax = 84.0
    ...
    # Add a shapefile
    shapefile = list(shpreader.Reader('../../shared/ne_10m_admin_1_states_provinces.shp').geometries())
