...
This document holds the strategic service-design framing for Grenada Meteorological Service digital work. It is deliberately broader than the current Figma-to-code bridge.

## Current Boundary

`GrenMet` now survives only in the Figma artifact names; the repository side is brand-neutral after transition boundaries 3-5. `GMS` is the service and product strategy those implementation artifacts support.
