/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import type { Options, RequestResult, Unwrappable } from "../.kubb/client.js";
import { client, withUnwrap } from "../.kubb/client.js";
import type {
  HrListShiftCatalogOptions,
  HrListShiftCatalogResponses,
} from "../models/HrListShiftCatalog.js";

/**
 * @description Return the shift catalog for roster assignment. Pass include_inactive=true (requires roster.manage) to include deactivated shift types for management screens.
 * @summary List shift catalog
 * {@link /api/v1/hr/rosters/shifts}
 */
export function hrListShiftCatalog<ThrowOnError extends boolean = true>(
  options: Options<HrListShiftCatalogOptions, ThrowOnError> = {}
): Unwrappable<RequestResult<HrListShiftCatalogResponses, ThrowOnError>> {
  const { client: request = client, ...config } = options;

  return withUnwrap(
    request({
      method: "GET",
      url: "/api/v1/hr/rosters/shifts",
      security: [{ type: "oauth2" }],
      ...config,
    }) as Promise<RequestResult<HrListShiftCatalogResponses, ThrowOnError>>
  );
}
