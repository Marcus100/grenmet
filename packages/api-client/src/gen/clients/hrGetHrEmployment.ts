/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import type { Options, RequestResult, Unwrappable } from "../.kubb/client.js";
import { client, withUnwrap } from "../.kubb/client.js";
import type {
  HrGetHrEmploymentOptions,
  HrGetHrEmploymentResponses,
} from "../models/HrGetHrEmployment.js";

/**
 * @description Return a user's employment record (department, status, position) so an admin can review or edit it. Supervisor or admin only.
 * @summary Get employment record (admin)
 * {@link /api/v1/hr/employment/:user_id}
 */
export function hrGetHrEmployment<ThrowOnError extends boolean = true>(
  options: Options<HrGetHrEmploymentOptions, ThrowOnError>
): Unwrappable<RequestResult<HrGetHrEmploymentResponses, ThrowOnError>> {
  const { client: request = client, ...config } = options;

  return withUnwrap(
    request({
      method: "GET",
      url: "/api/v1/hr/employment/{user_id}",
      security: [{ type: "oauth2" }],
      ...config,
    }) as Promise<RequestResult<HrGetHrEmploymentResponses, ThrowOnError>>
  );
}
