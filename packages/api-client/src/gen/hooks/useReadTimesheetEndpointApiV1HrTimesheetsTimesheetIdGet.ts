/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import type {
  QueryClient,
  QueryKey,
  QueryObserverOptions,
  UseQueryResult,
} from "@tanstack/react-query";
import { queryOptions, useQuery } from "@tanstack/react-query";
import type {
  Client,
  RequestConfig,
  ResponseErrorConfig,
} from "../../client.js";
import { readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet } from "../clients/readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet.js";
import type {
  ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet403,
  ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet404,
  ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet422,
  ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetPathParams,
  ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
} from "../models/ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet.js";

export const readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey = (
  timesheet_id: ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetPathParams["timesheet_id"]
) =>
  [
    {
      url: "/api/v1/hr/timesheets/:timesheet_id",
      params: { timesheet_id },
    },
  ] as const;

export type ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey =
  ReturnType<
    typeof readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey
  >;

export function readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryOptions(
  timesheet_id: ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetPathParams["timesheet_id"],
  config: Partial<RequestConfig> & { client?: Client } = {}
) {
  const queryKey =
    readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey(timesheet_id);
  return queryOptions<
    ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
    ResponseErrorConfig<
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet403
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet404
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet422
    >,
    ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
    typeof queryKey
  >({
    enabled: !!timesheet_id,
    queryKey,
    queryFn: async ({ signal }) => {
      if (!config.signal) {
        config.signal = signal;
      }
      return readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet(
        timesheet_id,
        config
      );
    },
  });
}

/**
 * @description Return a timesheet and its entries. Owner or user with timesheet.read.department over the owner.
 * @summary Get timesheet details
 * {@link /api/v1/hr/timesheets/:timesheet_id}
 */
export function useReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet<
  TData = ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
  TQueryData = ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
  TQueryKey extends
    QueryKey = ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey,
>(
  timesheet_id: ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetPathParams["timesheet_id"],
  options: {
    query?: Partial<
      QueryObserverOptions<
        ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryResponse,
        ResponseErrorConfig<
          | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet403
          | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet404
          | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet422
        >,
        TData,
        TQueryData,
        TQueryKey
      >
    > & { client?: QueryClient };
    client?: Partial<RequestConfig> & { client?: Client };
  } = {}
) {
  const { query: queryConfig = {}, client: config = {} } = options ?? {};
  const { client: queryClient, ...queryOptions } = queryConfig;
  const queryKey =
    queryOptions?.queryKey ??
    readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryKey(timesheet_id);

  const query = useQuery(
    {
      ...readTimesheetEndpointApiV1HrTimesheetsTimesheetIdGetQueryOptions(
        timesheet_id,
        config
      ),
      queryKey,
      ...queryOptions,
    } as unknown as QueryObserverOptions,
    queryClient
  ) as UseQueryResult<
    TData,
    ResponseErrorConfig<
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet403
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet404
      | ReadTimesheetEndpointApiV1HrTimesheetsTimesheetIdGet422
    >
  > & { queryKey: TQueryKey };

  query.queryKey = queryKey as TQueryKey;

  return query;
}
