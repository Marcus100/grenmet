/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import type {
  QueryClient,
  QueryKey,
  UseSuspenseQueryOptions,
  UseSuspenseQueryResult,
} from "@tanstack/react-query";
import { queryOptions, useSuspenseQuery } from "@tanstack/react-query";
import type {
  Client,
  RequestConfig,
  ResponseErrorConfig,
} from "../../client.js";
import fetch from "../../client.js";
import { readPermissionApiV1AuthPermissionsPermissionIdGet } from "../clients/readPermissionApiV1AuthPermissionsPermissionIdGet.js";
import type {
  ReadPermissionApiV1AuthPermissionsPermissionIdGet404,
  ReadPermissionApiV1AuthPermissionsPermissionIdGet422,
  ReadPermissionApiV1AuthPermissionsPermissionIdGetPathParams,
  ReadPermissionApiV1AuthPermissionsPermissionIdGetQueryResponse,
} from "../models/ReadPermissionApiV1AuthPermissionsPermissionIdGet.js";

export const readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey =
  (
    permission_id: ReadPermissionApiV1AuthPermissionsPermissionIdGetPathParams["permission_id"]
  ) =>
    [
      {
        url: "/api/v1/auth/permissions/:permission_id",
        params: { permission_id: permission_id },
      },
    ] as const;

export type ReadPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey =
  ReturnType<
    typeof readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey
  >;

export function readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryOptions(
  permission_id: ReadPermissionApiV1AuthPermissionsPermissionIdGetPathParams["permission_id"],
  config: Partial<RequestConfig> & { client?: Client } = {}
) {
  const queryKey =
    readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey(
      permission_id
    );
  return queryOptions<
    ReadPermissionApiV1AuthPermissionsPermissionIdGetQueryResponse,
    ResponseErrorConfig<
      | ReadPermissionApiV1AuthPermissionsPermissionIdGet404
      | ReadPermissionApiV1AuthPermissionsPermissionIdGet422
    >,
    ReadPermissionApiV1AuthPermissionsPermissionIdGetQueryResponse,
    typeof queryKey
  >({
    enabled: !!permission_id,
    queryKey,
    queryFn: async ({ signal }) => {
      if (!config.signal) {
        config.signal = signal;
      }
      return readPermissionApiV1AuthPermissionsPermissionIdGet(
        permission_id,
        config
      );
    },
  });
}

/**
 * @description Return a permission by ID (superuser only).
 * @summary Get permission by ID
 * {@link /api/v1/auth/permissions/:permission_id}
 */
export function useReadPermissionApiV1AuthPermissionsPermissionIdGetSuspense<
  TData = ReadPermissionApiV1AuthPermissionsPermissionIdGetQueryResponse,
  TQueryKey extends
    QueryKey = ReadPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey,
>(
  permission_id: ReadPermissionApiV1AuthPermissionsPermissionIdGetPathParams["permission_id"],
  options: {
    query?: Partial<
      UseSuspenseQueryOptions<
        ReadPermissionApiV1AuthPermissionsPermissionIdGetQueryResponse,
        ResponseErrorConfig<
          | ReadPermissionApiV1AuthPermissionsPermissionIdGet404
          | ReadPermissionApiV1AuthPermissionsPermissionIdGet422
        >,
        TData,
        TQueryKey
      >
    > & { client?: QueryClient };
    client?: Partial<RequestConfig> & { client?: Client };
  } = {}
) {
  const { query: queryConfig = {}, client: config = {} } = options ?? {};
  const { client: queryClient, ...queryOptions } = queryConfig;
  const queryKey =
    queryOptions?.queryKey ??
    readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryKey(
      permission_id
    );

  const query = useSuspenseQuery(
    {
      ...readPermissionApiV1AuthPermissionsPermissionIdGetSuspenseQueryOptions(
        permission_id,
        config
      ),
      queryKey,
      ...queryOptions,
    } as unknown as UseSuspenseQueryOptions,
    queryClient
  ) as UseSuspenseQueryResult<
    TData,
    ResponseErrorConfig<
      | ReadPermissionApiV1AuthPermissionsPermissionIdGet404
      | ReadPermissionApiV1AuthPermissionsPermissionIdGet422
    >
  > & { queryKey: TQueryKey };

  query.queryKey = queryKey as TQueryKey;

  return query;
}
