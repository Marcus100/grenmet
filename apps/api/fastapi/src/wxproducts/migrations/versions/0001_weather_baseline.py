...
def fingerprint(connection: Connection, schema: str) -> list[list[tuple[Any, ...]]]:
    queries = [
        """SELECT c.relname, a.attname, format_type(a.atttypid,a.atttypmod),
           ...
           AND c.relname != 'wxproducts_alembic_version'
           ORDER BY c.relname,a.attnum""",
        """SELECT c.relname, k.conname, pg_get_constraintdef(k.oid)
           FROM pg_constraint k JOIN pg_class c ON c.oid=k.conrelid
           JOIN pg_namespace n ON n.oid=c.relnamespace
           WHERE n.nspname=:schema AND c.relname != 'wxproducts_alembic_version'
           ORDER BY c.relname,k.conname""",
        """SELECT tablename,indexname,indexdef FROM pg_indexes WHERE schemaname=:schema
           AND tablename != 'wxproducts_alembic_version' ORDER BY tablename,indexname""",
...
def upgrade() -> None:
    ...
    tables = (
        connection.execute(
            text(
                "SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename != 'wxproducts_alembic_version'"
            )
        )
        .scalars()
        .all()
