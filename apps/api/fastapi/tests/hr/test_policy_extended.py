...
async def test_user_without_role_denied(db_async: AsyncSession) -> None:
    ...
    assert has_permission(current_user=user, permission_key="leave.request.create.self") is False
...
async def test_user_with_matching_permission_allowed(db_async: AsyncSession) -> None:
    ...
    assert has_permission(current_user=user, permission_key="leave.request.create.self") is True


async def test_require_permission_raises_for_missing_key(db_async: AsyncSession) -> None:
...
async def test_require_permission_does_not_raise_for_superuser(db_async: AsyncSession) -> None:
