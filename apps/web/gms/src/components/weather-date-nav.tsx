      ...
      {days.map((day) => {
        ...
        return (
          ...
          >
            {isActive && (
              <motion.div
                className="absolute inset-0 border-2 border-gm-navy bg-background lg:border-x-0 lg:border-t-0 lg:border-r-[3px] lg:border-r-gm-navy lg:border-b-0"
                layoutId="weather-date-nav-active"
                transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
              />
            )}
            <Link
              className={cn(
                "relative flex flex-col items-center justify-center gap-1 px-1 py-2 text-center lg:py-3",
                isActive ? "text-gm-navy" : "bg-gm-surface text-gm-text-muted"
              )}
              href={href}
            >
              ...
              {/* The month is dropped — a five-day strip never spans enough
                  to need it, and the date carries the emphasis instead. */}
